  ...
  */
  ...
  Point.prototype = {
    ...
    angleBetween: function (p1, p2) {
      var angleBetween =
        this.equals(p1) || this.equals(p2)
          ? NaN
          : this.theta(p2) - this.theta(p1);
    ...
    cross: function (p1, p2) {
      return p1 && p2
        ? (p2.x - this.x) * (p1.y - this.y) - (p2.y - this.y) * (p1.x - this.x)
        : NaN;
    },
    ...
    move: function (ref, distance) {
      ...
      var offset = this.offset(
        cos$1(theta) * distance,
        -sin$1(theta) * distance
      );
      return offset;
    },
  ...
  Line.prototype = {
    ...
    closestPointNormalizedLength: function (p) {
      ...
      var cpNormalizedLength = min$1(
        1,
        max$1(0, product / this.squaredLength())
      );
    ...
    intersectionWithLine: function (line) {
      var pt1Dir = new Point(
        this.end.x - this.start.x,
        this.end.y - this.start.y
      );
      var pt2Dir = new Point(
        line.end.x - line.start.x,
        line.end.y - line.start.y
      );
      var det = pt1Dir.x * pt2Dir.y - pt1Dir.y * pt2Dir.x;
      var deltaPt = new Point(
    ...
    pointAtLength: function (length) {
      ...
      return this.pointAt(
        (fromStart ? length : lineLength - length) / lineLength
      );
    },
  ...
  Ellipse.prototype = {
    ...
    tangentTheta: function (p) {
      ...
      if (q1 || q3) {
        ...
        x =
          (a * a) / (x0 - m) -
          (a * a * (y0 - n) * (y - n)) / (b * b * (x0 - m)) +
          m;
      } else {
        x = y0 > center.y ? x0 + refPointDelta : x0 - refPointDelta;
        y =
          (b * b) / (y0 - n) -
          (b * b * (x0 - m) * (x - m)) / (a * a * (y0 - n)) +
          n;
      }
  ...
  Rect.prototype = {
    ...
    intersect: function (r) {
      ...
      return new Rect(
        x,
        y,
        min$2(myCorner.x, rCorner.x) - x,
        min$2(myCorner.y, rCorner.y) - y
      );
    },
  ...
  function convexHull(points) {
    ...
    while (sortedPointRecords.length !== 0) {
      ...
      while (!correctTurnFound) {
        if (hullPointRecords.length < 2) {
          ...
          var crossProduct = secondLastHullPoint.cross(
            lastHullPoint,
            currentPoint
          );
          ...
          if (crossProduct < 0) {
            ...
            var angleBetween = lastHullPoint.angleBetween(
              secondLastHullPoint,
              currentPoint
            );
    ...
    if (indexOfLowestHullIndexRecord > 0) {
      ...
      var newSecondChunk = hullPointRecords.slice(
        0,
  ...
  Curve.prototype = {
    ...
    closestPointLength: function (p, opt) {
      opt = opt || {};
      var precision =
        opt.precision === undefined ? this.PRECISION : opt.precision;
      var subdivisions =
        opt.subdivisions === undefined
          ? this.getSubdivisions({ precision: precision })
          : opt.subdivisions;
    ...
    closestPointT: function (p, opt) {
      ...
      for (var i = 0; i < n; i++) {
        ...
        if (!minSumDist || sumDist < minSumDist) {
          ...
          chordLength = currentSubdivision.start.distance(
      ...
      while (true) {
        ...
        var startPrecisionRatio = distFromStart
          ? abs$2(distFromStart - distFromEnd) / distFromStart
          : 0;
        var endPrecisionRatio = distFromEnd
          ? abs$2(distFromStart - distFromEnd) / distFromEnd
          : 0;
        ...
        var hasMinimalStartDistance = distFromStart
          ? distFromStart < chordLength * precisionRatio
          : true;
        var hasMinimalEndDistance = distFromEnd
          ? distFromEnd < chordLength * precisionRatio
          : true;
        var hasMinimalDistance =
          hasMinimalStartDistance || hasMinimalEndDistance;
    ...
    getSubdivisions: function (opt) {
      ...
      while (true) {
        ...
        var observedPrecisionRatio =
          length !== 0 ? (length - previousLength) / length : 0;
    ...
    length: function (opt) {
      opt = opt || {};
      var precision =
        opt.precision === undefined ? this.PRECISION : opt.precision; // opt.precision only used in getSubdivisions() call
      var subdivisions =
        opt.subdivisions === undefined
          ? this.getSubdivisions({ precision: precision })
          : opt.subdivisions;
    ...
    tAtLength: function (length, opt) {
      ...
      while (true) {
        ...
        observedPrecisionRatio =
          curveLength !== 0 ? baselinePointDistFromStart / curveLength : 0;
        if (observedPrecisionRatio < precisionRatio) {
          return investigatedSubdivisionStartT;
        }
        observedPrecisionRatio =
          curveLength !== 0 ? baselinePointDistFromEnd / curveLength : 0;
        if (observedPrecisionRatio < precisionRatio) {
  ...
  var Path = function (arg) {
    ...
    if (!arg);
      ...
      if (arg[0].isSegment) {
        ...
        for (i = 0; i < n; i++) {
          ...
          if (!(obj instanceof Line || obj instanceof Curve)) {
            throw new Error(
              "Cannot construct a path segment from the provided object."
            );
          }
      ...
      this.appendSegment(Path.createSegment("M", arg.start));
      this.appendSegment(
        Path.createSegment("C", arg.controlPoint1, arg.controlPoint2, arg.end)
      );
  ...
  Path.prototype = {
    ...
    appendSegment: function (arg) {
      ...
      var previousSegment =
        numSegments !== 0 ? segments[numSegments - 1] : null; // if we are appending to an empty path, previousSegment is null
    ...
    closestPointLength: function (p, opt) {
      ...
      var segmentSubdivisions =
        opt.segmentSubdivisions === undefined
          ? this.getSegmentSubdivisions({ precision: precision })
    ...
    divideAtLength: function (length, opt) {
      ...
      pathCopy.insertSegment(
        divisionMidIndex,
        Path.createSegment("M", movetoEnd)
      );
      divisionEndIndex += 1;
      ...
      for (i = divisionEndIndex; i < pathCopy.segments.length; i++) {
        var originalSegment = this.getSegment(
    ...
    equals: function (p) {
      ...
      for (var i = 0; i < numSegments; i++) {
        ...
        if (
          segment.type !== otherSegment.type ||
          !segment.equals(otherSegment)
        ) {
          return false;
        }
      }
    ...
    length: function (opt) {
      ...
      opt = opt || {};
      var precision =
        opt.precision === undefined ? this.PRECISION : opt.precision; // opt.precision only used in getSegmentSubdivisions() call
      var segmentSubdivisions =
        opt.segmentSubdivisions === undefined
          ? this.getSegmentSubdivisions({ precision: precision })
          : opt.segmentSubdivisions;
      ...
      for (var i = 0; i < numSegments; i++) {
        ...
        if (segment.isVisible) {
          ...
          if (currentSegmentSubdivisions.length > 0) {
            var subdivisionPoints = currentSegmentSubdivisions.map(function (
              curve
            ) {
              return curve.start;
            });
  ...
  var segmentPrototype = {
    ...
    divideAtT: function (t) {
      ...
      throw new Error(
        "Neither divideAtT() nor divideAt() function is implemented."
      );
    },
    ...
    pointAtT: function (t) {
      ...
      throw new Error(
        "Neither pointAtT() nor pointAt() function is implemented."
      );
    },
    ...
    tangentAtT: function (t) {
      ...
      throw new Error(
        "Neither tangentAtT() nor tangentAt() function is implemented."
      );
    },
  ...
  Path.regexSupportedData = new RegExp(
  ...
  function rectWithLine(rect, line) {
    ...
    return (
      lineWithLine(rect.topLine(), line) ||
  ...
  function _polylineWithPolygon(polyline, polygon, opt) {
    return (
      polygon.containsPoint(polyline.start) ||
      _polylineWithPolyline(polyline, polygon.clone().close(), opt)
    );
  }
